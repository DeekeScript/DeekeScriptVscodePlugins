...
const DOC_BASE = 'https://doc.deeke.cn';
...
function buildMethodDocUrl(globalName: string, method: MethodDef): string {
    ...
    const anchor = `${method.name}${method.params.map(p => p.name).join('')}`.toLowerCase();
    return `${base}#${anchor}`;
}
...
function buildMethodJSDoc(globalName: string, method: MethodDef): string[] {
    ...
    docLines.push(`@see {@link ${buildMethodDocUrl(globalName, method)} DeekeScript 文档}`);
...
function buildPropertyJSDoc(globalName: string, prop: PropertyDef): string[] {
    ...
    docLines.push(`@see {@link ${buildDocUrl(globalName)} DeekeScript 文档}`);
    return buildJSDocLines(docLines);
}
...
function buildFunctionJSDoc(def: GlobalDef, name: string): string[] {
    ...
    docLines.push(`@see {@link ${buildDocUrl(name)} DeekeScript 文档}`);
    return buildJSDocLines(docLines);
}
...
function buildConstructorJSDoc(def: GlobalDef, className: string): string[] {
    ...
    docLines.push(`@see {@link ${buildDocUrl(className)} DeekeScript 文档}`);
    return buildJSDocLines(docLines);
}
        ...
        switch (def.kind) {
            ...
            case 'function': {
                ...
                if (def.methods.length > 0 && returnType && returnType !== 'void' && !declared.has(returnType)) {
                    ...
                    appendJSDocLines(lines, [`${returnType} 类型`, `@see {@link ${buildDocUrl(name)} DeekeScript 文档}`], '');
